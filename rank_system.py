...
# --- FIX: Changed arguments to (user_id, users_col, msg=None) to match app.py ---
def process_daily_rewards(user_id, users_col, msg=None):
    """
    Handles daily rewards.
    If 'msg' is None, it acts as a silent login check (returns empty).
    If 'msg' is provided (journal entry), it calculates rewards.
    """
    if users_col is None: return {'awarded': False}

    # If called from login (no message), we can return early or just check streak
    if msg is None:
        return {} 
...
# --- FIX: Changed arguments to (user_id, users_col, history_col=None) and returns Tuple ---
def update_rank_check(user_id, users_col, history_col=None):
    """
    Checks if current stardust is enough to Level Up.
    Returns Tuple (RankName, Message) to match app.py expectations.
    """
    ...
    if current_idx >= len(RANK_SYSTEM):
        current_idx = len(RANK_SYSTEM) - 1
    ...
    if current_dust >= req and current_idx < len(RANK_SYSTEM) - 1:
        ...
        current_idx += 1
        new_rank_data = RANK_SYSTEM[current_idx]
        new_rank = new_rank_data['title']

        users_col.update_one({"user_id": user_id}, {
            "$set": {"rank_index": current_idx, "rank": new_rank, "stardust": new_dust}
        })
        
        return new_rank, f"Ascended to {new_rank}!"
    
    return None, None

# --- FIX: Updated to handle direct XP input or Index ---
def get_rank_meta(val):
    """
    Returns (Title, StarType, Req) tuple for app.py.
    'val' can be an index (int) or ignored if looking up by index is handled elsewhere.
    Since app.py passes 'xp', we treat 'val' as the Rank Index.
    """
    idx = int(val) # Convert to int just in case
    if idx < 0: idx = 0
    if idx >= len(RANK_SYSTEM): idx = len(RANK_SYSTEM) - 1

    data = RANK_SYSTEM[idx]

    # Inject Assets
    base_name = data['title'].split(' ')[0]
    
    # Return Tuple expected by app.py: (Title, Psyche/Type, Req)
    return data['title'], data['psyche'], data['req']
