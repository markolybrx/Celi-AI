...
try:
    ...
    # Define dummy functions so app doesn't crash if tasks.py is broken
...
def generate_immediate_reply(msg):
    try:
        ...
        payload = { "contents": [{ "parts": [{"text": f"You are Celi, a warm AI journal companion. Respond to this in 2 sentences: {msg}"}] }] }
...
def process():
    ...
    try:
        ...
        # Try background tasks safely
        if process_entry_analysis: process_entry_analysis.delay(timestamp, msg, session['user_id'])
        if generate_weekly_insight: generate_weekly_insight.delay(session['user_id'])
        ...
        print(f"Error: {e}")
        return jsonify({"reply": "Signal Lost."}), 500
