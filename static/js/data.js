// --- DATA & CALENDAR LOGIC (INSTANT LOAD VERSION) ---
...
async function loadData() { 
    // 1. FAST: Load from cache immediately if available
    const cachedData = localStorage.getItem('celi_user_data');
    if (cachedData) {
        console.log("⚡ Instant Load from Cache");
        renderData(JSON.parse(cachedData));
    }

    // 2. SLOW: Fetch fresh data from server in background
    try { 
        ...
        // 3. UPDATE: Refresh UI and Cache
        localStorage.setItem('celi_user_data', JSON.stringify(data));
        console.log("🔄 Data Refreshed from Server");
        renderData(data); // Re-render with fresh data

    } catch(e) { 
        console.error("Network Error, utilizing cache:", e); 
    } 
}

// RENDER FUNCTION (Used by both Cache and Network)
function renderData(data) {
    ...
    if(dot) dot.style.backgroundColor = data.aura_color || data.current_color || '#00f2fe';
...
function renderCalendar() { 
    ...
    for(let i=1; i<=days; i++) { 
        ...
        if (dateMap[dateKey]) {
            ...
            d.onclick = (e) => { e.stopPropagation(); openArchive(entryData.id); };
        }
        g.appendChild(d); 
    } 
}
...
function toggleDatePicker() { const picker = document.getElementById('cal-picker'); if (!picker) return; if (!picker.classList.contains('active')) { pickerDate = new Date(currentCalendarDate.getTime()); selectedMonthIdx = pickerDate.getMonth(); isYearMode = false; renderPickerUI(); picker.classList.add('active'); } else { picker.classList.remove('active'); } }
function toggleYearMode() { isYearMode = !isYearMode; renderPickerUI(); }
function renderPickerUI() { const yearText = document.getElementById('picker-year-text'); const monthsContainer = document.getElementById('picker-months-container'); const yearsContainer = document.getElementById('picker-years-container'); yearText.innerText = pickerDate.getFullYear(); if (isYearMode) { monthsContainer.classList.add('hidden'); yearsContainer.classList.remove('hidden'); yearsContainer.innerHTML = ''; const currentYear = new Date().getFullYear(); for (let y = currentYear - 15; y <= currentYear + 15; y++) { const btn = document.createElement('div'); btn.className = `picker-year-btn ${y === pickerDate.getFullYear() ? 'selected' : ''}`; btn.innerText = y; btn.onclick = () => { pickerDate.setFullYear(y); toggleYearMode(); }; yearsContainer.appendChild(btn); } setTimeout(() => { const selected = yearsContainer.querySelector('.selected'); if(selected) selected.scrollIntoView({block: 'center'}); }, 10); } else { yearsContainer.classList.add('hidden'); monthsContainer.classList.remove('hidden'); monthsContainer.innerHTML = ''; ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"].forEach((m, idx) => { const btn = document.createElement('div'); btn.className = `picker-month-btn ${idx === selectedMonthIdx ? 'selected' : ''}`; btn.innerText = m; btn.onclick = () => { selectedMonthIdx = idx; pickerDate.setMonth(idx); renderPickerUI(); }; monthsContainer.appendChild(btn); }); } }
function confirmDateSelection() { currentCalendarDate.setFullYear(pickerDate.getFullYear()); currentCalendarDate.setMonth(selectedMonthIdx); renderCalendar(); toggleDatePicker(); }